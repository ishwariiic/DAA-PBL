...
function initializeApp() {
    ...
    algoItems.forEach((item, index) => {
        item.addEventListener('click', () => {
            ...
            const algoId = item.getAttribute('data-algo');
            const algoIndex = algorithms.findIndex(algo => algo.id === algoId);
            
            if (algoIndex !== -1) {
                setActiveAlgorithm(algoIndex);
            }
        });
    });
...
function updateAlgorithmDetails() {
    ...
    if (!pseudocodeElement || !codeSnippetElement || !codeContent) {
        console.error('Required code elements not found in the DOM');
        return;
    }
    
    try {
        // Get code content
        const pseudocode = algorithm.pseudocode || '// No pseudocode available';
        const codeSnippet = algorithm.codeSnippet || '// No code available';
        
        // Update pseudocode section
        const pseudoPre = document.createElement('pre');
        const pseudoCode = document.createElement('code');
        pseudoCode.className = 'language-python';
        pseudoCode.textContent = pseudocode;
        pseudoPre.appendChild(pseudoCode);
        pseudocodeElement.innerHTML = '';
        pseudocodeElement.appendChild(pseudoPre);
        
        // Update code snippet section
        const snippetPre = document.createElement('pre');
        const snippetCode = document.createElement('code');
        snippetCode.className = 'language-javascript';
        snippetCode.textContent = codeSnippet;
        snippetPre.appendChild(snippetCode);
        codeSnippetElement.innerHTML = '';
        codeSnippetElement.appendChild(snippetPre);
        
        // Update main code display (tabbed view)
        const mainPre = document.createElement('pre');
        const mainCode = document.createElement('code');
        
        if (currentCodeLang === 'pseudocode') {
            mainCode.className = 'language-python';
            mainCode.textContent = pseudocode;
        } else {
            mainCode.className = 'language-javascript';
            mainCode.textContent = codeSnippet;
        }
        
        mainPre.appendChild(mainCode);
        codeContent.innerHTML = '';
        codeContent.appendChild(mainPre);
        
        // Apply syntax highlighting to all code blocks
        applySyntaxHighlighting();
    } catch (error) {
        console.error('Error updating algorithm details:', error);
    }
}
...
function setActiveCodeTab(lang) {
    ...
    // Set appropriate content based on selected tab
    if (lang === 'pseudocode') {
        const pseudocode = algorithm.pseudocode || '// No pseudocode available';
        code.className = 'language-python';
        code.textContent = pseudocode;
    } else {
        const codeSnippet = algorithm.codeSnippet || '// No code available';
...
function setActiveAlgorithm(index) {
    ...
    algoItems.forEach(item => {
        if (item.getAttribute('data-algo') === algorithm.id) {
            item.classList.add('active');
        } else {
